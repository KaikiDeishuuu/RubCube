  ...
  cube-core/          # 零依赖纯 TS。浏览器 + Node 共用
    src/
      ...
      metrics/
        ...
        structural.ts # §6.5 的 progress_score（归位块数）与 cubie 正确性
        score.ts      # 两版指标共用的 clip、best_progress 与 coverage 记账
        proxy.ts      # 被否决的解长代理，保留供对照；两个 ratio 仍在用
        statistics.ts # Spearman ρ、MAE、逆序、Lipschitz 违规率
        manifest.ts   # 两轮 M3d 预注册 profile、语料、阈值与 fingerprint
    ...
    scripts/
      ...
      validate-distance-proxy.mjs # M3d 第一轮：距离代理验证 profile（判定 no-go）
      validate-progress-metric.mjs # M3d 第二轮：结构进度验证 profile（判定 go）
...
截至 2026-08-20，`/solver`（M3a 表层 + M3b 搜索）、`/optimal`（M3c）与 `/metrics`（M3d）都已落地；`/tutorial` 仍属于后续里程碑。`/metrics` 提供 `progress_score` 的结构进度实现（M3d 第二轮通过）以及被否决的解长代理（保留供对照），见 §6.5。生产 app 的主 bundle 必须继续保持不含 `/solver` 与 `/optimal`：`/solver` 只出现在 solver worker chunk 里，`/optimal` 一个 chunk 都不许出现，后者由 `packages/app/build/forbid-node-only-modules.ts` 在生产构建中遍历 Rollup 模块图断言。
...
**`isSolved` 是朝向无关的**：判据是"六个面各自单色"，而不是"等于标准复原态"。玩家用切片把整个魔方转过之后，`cp/co/ep/eo/centers` 都不等于恒等，但六面依然单色——那就是复原了，只是朝着另一个方向。实现上逐贴纸比较它所在面的中心颜色（中心 `i` 的颜色就是 `i`，所以全是整数比较），48 个贴纸一起覆盖了置换与朝向，不需要另跑一遍 cp/co/ep/eo。只有面转时这个判据与旧的"等于标准复原态"完全一致，因为那时中心恒等。
...
**键盘**：采用 csTimer 系风格的速拧键位，六个面加三个中层共 20 个键，映射逐键抄自 csTimer 的 `generateCubeKeyMapping`：

| | 顺时针 | 逆时针 |
|---|---|---|
| U / R / L / F / D / B | J · I · E · H · S · W | F · K · D · G · L · O |
| M / E / S | 5 · 6（M）· 2（E）· 0（S） | `.` · X（M'）· 9（E'）· 1（S'） |

M 给了左右手各一个键，E 和 S 各只有一个——这是 csTimer 自己的不对称，照抄而不是自己补。**没有双转键**，U2 就是 J 按两下，同样跟 csTimer。

**绑定的是 `KeyboardEvent.code`（物理位置），不是 `key`（字符）。** csTimer 用的是更老的 `keyCode`，所以它的键位在 Dvorak / AZERTY / Colemak 上会散开——U、R、R' 三个键在 Dvorak 上全跑到左手，这个问题在上游挂了多年（cs0x7f/cstimer#72）。按 `code` 绑定，右手食指下面那颗键永远是右手食指下面那颗键。副作用是界面上显示的字母不再由绑定本身决定：默认显示 QWERTY 字母，能拿到 `navigator.keyboard.getLayoutMap()`（Chromium）时显示键帽真实字符。

映射可在面板里逐键重绑（点键帽 → 按键 → 生效，Esc 取消），存进 `localStorage`，存的是走法记号而不是对象，读回时过一遍同一个解析器；任何一条无效就整份退回默认。Space、Esc、Tab、Enter 和各修饰键保留给应用，不接受绑定——尤其是修饰键，转动处理器本来就会忽略带修饰的按键，绑上去会「看起来成功、按下去没反应」。**重绑一个走法会把它原有的键全部收拢到刚按下的那一个**（例如 M' 原本有 `.` 和 X 两个键），恢复默认一键可回。
...
**停止条件是「魔方复原」，不是「任意键」。** WCA 的任意键规则前提是键盘*就是*计时器；这里键盘是用来转魔方的（`J/F/I/K/…`，见 §4.6），照搬会让一次解法的第一步就把表停掉。判定走 `isSolved`，它是朝向无关的，所以收尾时整体转了个方向仍然算解出。放弃本次尝试用 `Esc`，记为 DNF。

蓄力有两条入口：空格，以及面板上的按压区。手机没有空格键，只留键盘入口等于在触屏上根本无法计时。**指针被夺走（`pointercancel`）和窗口失焦不能当作「松手开始」**——那会启动一个没人在看的计时；它们走单独的取消事件，只丢掉蓄力，观察倒计时继续。

- 可选 15 秒观察时间：超时 +2，超 17 秒 DNF。观察期间**禁止转动**（键盘与拖拽都拦），否则既违规、也会让计分对应的状态和实际打乱不一致
- **计时真相源是 `performance.now()`**，不是 rAF 时间戳。显示更新走 rAF，但两者解耦
- 结束时刻在 transport 的 commit 回调里读取，而不是在 React effect 里。commit 就是转层动画结束的那一帧，晚一个渲染周期取值会把调度器的延迟算进成绩
- 蓄力推进由 rAF 驱动而非 `setTimeout`，这样标签页不可见时不会在无人看着的情况下把计时器蓄满
- 结果落地：`{ id, recordedAt, rawMs, penalty, scramble, scrambleSeed }`。`rawMs` 存原始值，+2 在读取时施加，这样事后改判不会丢掉原始成绩

### 5.3 统计

- 单次、best、mean、ao5、ao12、ao100，每个平均同时给「当前」和「本次会话最佳」
- 修剪量取 `max(1, round(N/20))`：ao5/ao12 各去掉一头一尾，ao100 各去 5%。WCA 9f 只定义了 5 次的情形，长窗口沿用 csTimer 的比例，否则一次崩盘就能主导一个本该描述整段会话的数字
- DNF 排序为「最差」，因此会先被修剪吃掉；**DNF 数量超过修剪量时，平均本身就是 DNF**——没有时间可以代入，而直接丢弃则等于把失败的尝试当成没做过
- `mean` 只统计出成绩的尝试，并同时给出 `solved / count`，避免一段险些全灭的会话看起来很干净
- 罚时存储上分离：记录里存 `rawMs` 原始值，+2 在读取时施加。事后改判不会破坏原始成绩
- 本地 IndexedDB 存储。整段会话写为单条记录而不是一条一记录：store 是唯一真相源，而按条写必须对它做 diff，那种簿记会悄悄漂移。写入去抖 400ms，并在标签页隐藏时立即冲刷
- 读回时逐条校验；坏记录只损失该条，不会让整段练习会话陪葬。恢复后按 `recordedAt` 重排——平均是连续尝试上的滑动窗口，顺序变了同样一批成绩会算出不同的数
- 导出 CSV：列集与分隔符取 csTimer 的形状，但**未经 csTimer 实际导入验证**，不要当作已认证的格式

**尚未实现：** 多会话（命名/切换/分别统计）。当前只有单一持久会话。
...
| **`progress_score`** | `(placed(s_final) - placed(s_0)) / (20 - placed(s_0))`，clip 到 [0,1] | **核心细粒度指标**：`placed` = 位置与朝向都正确的块数。M3d 第二轮预注册验证通过；第一版的 Kociemba 解长代理已被否决，见下 |
...
| `kociemba_ratio` | `htm_count / kociemba_baseline_len`（k>9） | 相对非最优基线的效率；可能 < 1，不能称为最优率。分母是求解器对**同一个起始状态**给出的解长，M3d 否决的是拿它当跨状态距离用，不是拿它当固定基线 |
...
**M3d 第一轮的实验已经做完，判定是否决。** 预注册 manifest（`cube-core/src/metrics/manifest.ts`，`sha256:0781cb57…`）的八条门槛过六条，两条不过：逆序率 0.0591（标准 ≤ 0.05）、Lipschitz 违规率 0.1583（标准 ≤ 0.10）。原因有两条，都可以直接从数据读出：搜索在找到第一条 ≤ `targetLength` 的解时就返回，对接近复原的魔方常常给出远长于最优解的答案（真距离 9 时平均高估 3.68 步，最坏 12 步）；而在远端代理直接饱和——**14 步打乱与均匀随机状态的代理中位数都是 21，分不开**。最直白的一组数字来自第二轮的层级语料：整整一个第一层做完，这个代理只从 20.68 走到 20.25，折算成 `progress_score` 约 0.02。

**`progress_score` 因此整体换成结构进度（`structural-cubies-v1`），第二轮预注册验证通过（`sha256:1f2639b4…`，六条门槛全过）。** 新定义：`placed(s)` 是二十个块里位置与朝向都正确的个数，判据与 `cubie_accuracy` 同一条；`progress_score(s0, s) = clip((placed(s) - placed(s0)) / (20 - placed(s0)), 0, 1)`，`placed(s0) = 20` 时为 null。中心块不计分但必须在原位，整体旋转过的魔方直接拒绝。它在同一批层级状态上给出 0.040 / 0.243 / 0.437 / 0.639 / 0.701 / 1.000，层级 Spearman ρ 0.9765（旧代理 0.8254），每次打分 0.0006 ms 且不需要表。已知短板同样预注册并如实报告：它在离复原十几步以外饱和在 0，替身轨迹类只报告不设门槛。

`optimality_ratio` 从头到尾不受影响，它的分母是 M3c 的真最优；`kociemba_ratio` 也不受影响，它是对同一个起始状态的固定基线，而 M3d 否决的是拿解长做跨状态比较。上面这一整段的 null / coverage 语义本身是成立的，两轮实测里 coverage 都是 100%，没有静默丢样本；被否决的是「用 Kociemba 解长当距离」这一件事，不是这套记账规则。完整数据见 DESIGN-SOLVING.md 的两节「M3d 实测」。
...
| **M3d** | 距离代理验证 profile：相关性、误差、逆序、局部一致性与 coverage | 两轮：**否决** Kociemba 解长代理，**通过** 结构进度实现，`progress_score` 启用 |
...
**M0 → M3a → M3b → M3d 是关键路径**：M3b 只提供候选距离代理，必须经过 M3d 才能进入正式指标。**M3d 两轮都已走完**——第一轮否决了那个代理，第二轮的结构进度实现通过了预注册验证——所以这条关键路径已经打通，M4 可以带着正式指标开发。M2.5 与 M3.5 扩展人类可玩目标，但不阻塞 benchmark；M3c 只阻塞短打乱的真最优校准，现已完成。
...
M3d 分两轮执行完毕。**第一轮否决**了 Kociemba 解长代理：三类语料 coverage 全 100%、Spearman ρ 0.9201、MAE 1.227、方向一致率 0.9611 都达标，但逆序率 0.0591 与 Lipschitz 违规率 0.1583 不达标，根因是首解即返回加上远端饱和。**第二轮通过**了替代实现 `structural-cubies-v1`（归位块数），六条门槛全过：25,640 个状态上 `score = 1` 与 `isSolved` 零分歧、层级严格单调、层级 ρ 0.9765、5,000 个均匀随机状态的均值 0.8282 命中闭式预测 20/24、10,000 对相邻状态零违规、并在同一批状态上强于被替换的指标（0.9765 vs 0.8254）。`/metrics` 同时保留两版实现与两份预注册 manifest，后者的散列文本里写着前者的指纹。完整数据见 DESIGN-SOLVING.md 的两节「M3d 实测」。
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| ~~Kociemba 表只有单机 Node 冷进程基线~~ 桌面浏览器已补齐（冷生成 P95 726 ms、缓存命中 123 ms），选定 Worker 生成 + IndexedDB；**移动设备仍未测** | 移动端冷生成若慢 4 倍会贴近 3 s 门槛，影响首次使用的等待 | 缓存路径把风险限制在首次访问；拿到移动端数据前不把「B 已验证」当成全平台结论 |
...
| ~~距离代理已被 M3d 否决~~ 已整体换成结构进度并通过第二轮预注册验证 | 换版本身已闭环；**残留风险是新指标只在构造语料上验证过，真实解题轨迹上的表现仍未知** | 报告里同时给出 `progress_score` 与它的层级刻度表，读者能看见每个里程碑值多少；M3.5 与 M4 落地后按 §9 待决 1 重跑轨迹类 |

**待决问题（需要先做实验才能定）：**

1. ~~`progress_score` 用 Kociemba 解长度是否足够可靠？~~ **已答：不够，M3d 第一轮判定 no-go**，逆序率 0.0591 > 0.05、Lipschitz 违规率 0.1583 > 0.10。~~由此产生的「换成什么」~~ **也已答：换成结构进度 `structural-cubies-v1`，M3d 第二轮预注册验证通过，六条门槛全过。** 两轮的 manifest、语料、诊断与偏差记录见 DESIGN-SOLVING.md 的两节「M3d 实测」。

   预设的「角块/棱块 PDB 下界」没有采用，因为同一份数据表明它更差：M3a 四张剪枝表的直径是 9/9/14/12 且每张只界定一个阶段，phase-1 下界在 9 就封顶；完整角块 PDB（88,179,840 条）在 HTM 下直径 11，也远低于 18。更根本的是球的层数每层约 ×13 而全群是 4.33 × 10¹⁹，绝大多数状态挤在 17、18 两层，**距离比值这个形状本身在真实任务上就没有动态范围**。结构进度不是距离，因此不受这条约束：它按「有多少块可验证地归位」计分，动态范围满、不依赖求解器、每次 0.0006 ms。

   **仍然开着的一条**：结构进度在离复原十几步以外饱和在 0，而验证它的轨迹语料是替身（走 Kociemba 解，那条路上直到最后几步没有块归位），所以「真实解题轨迹上它表现如何」这一类证据目前是空的。M3.5 能生成分层解法之后必须重跑该类；M4 拿到真实模型 trace 之后同样要重跑。在那之前，正式报告照常使用 `progress_score`，但不能声称它已在真实轨迹上验证过。
