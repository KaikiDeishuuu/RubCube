...
>
> 本文中的状态空间与表大小来自明确的坐标基数推导；球层数量、表直径和公式保持性是实现必须复现的验收常数。M3a 的六张移动表和四张剪枝表已由穷举 oracle 固化，9/9/14/12 是当前实现复现的直径；M3c 的球层数量 1/18/243/3,240/43,239/574,908 也已由实现复现。**M3d 的两轮验证都已跑完**：Kociemba 解长代理被否决，`structural-cubies-v1` 通过（见两节「M3d 实测」），所以 §6.5 的 `progress_score` 在本文里按「已启用、结构进度版」读。M3.5 教程仍未实现，不能把对应设计值提前写成实测结果。
...
cube-core (state / moves，默认入口只导出规则 API)
  ...
  ├── optimal/       ← M3c。k≤9 真最优；Node/bench only
  ├── tutorial/      ← 模块三。通过 /tutorial 子路径导出；只依赖规则 API
  └── metrics/       ← §6.5 的 progress_score（结构进度）与两轮 M3d 预注册 manifest，只给裁判使用
...
2026-08-17 的本机 Node 22 / WSL2 基线在 i7-9750H 上以三个独立冷进程得到：生成 851.36–884.85 ms（中位数 856.59 ms）、编码 82.40–85.59 ms（中位数 83.86 ms）、严格解码 70.91–75.09 ms（中位数 71.45 ms）。artifact 固定为 3,799,756 B，CRC32 为 `crc32:facd232e`，SHA-256 为 `319936a6f02b58d24c25340e29d08e313ee472656e22ac5acd742046130ba38c`；完整样本见 [`solver-tables-node-wsl2-2026-08-17.json`](packages/cube-core/benchmarks/solver-tables-node-wsl2-2026-08-17.json)。

**2026-08-20 浏览器实测：决定选 B。** headless Chromium（SwiftShader）/ i7-9750H / WSL2，**每轮换一个全新 profile 和全新浏览器进程**，在 Worker 里跑（产品里就跑在那儿；放主线程会阻塞整页，测出来的是不存在的情形），n=12：

| 阶段 | P50 | P95 | max |
|---|---|---|---|
| 冷生成 | 716.2 ms | **726.0 ms** | 742.0 ms |
| 序列化（encode） | 111.2 ms | 113.8 ms | 114.2 ms |
| 严格校验（decode） | 99.6 ms | 102.3 ms | 110.7 ms |
| IndexedDB 写入 | 49.7 ms | 51.5 ms | 52.8 ms |
| **缓存命中读取** | 11.3 ms | 14.7 ms | 17.1 ms |
| **缓存命中解码** | 112.1 ms | 117.2 ms | 129.7 ms |

artifact 大小 3,799,756 B，与 Node 基线逐字节一致。浏览器冷生成反而比 Node 基线（856.59 ms）快一些。

冷路径合计约 977 ms（P50），**冷生成 P95 726 ms 远低于 §2.6 的 3 s 门槛**；缓存路径 12/12 全部命中，命中后合计约 123 ms。按本节的判据 → **选方案 B：首次在 Worker 生成、存 IndexedDB。** 这也正是已实现的路径。

**这条证据只覆盖桌面浏览器。** 目标移动设备仍未测；移动端若比这台机器慢 4 倍，冷生成会落在 2.9 s 附近，也就是刚好贴着门槛。缓存路径让这个风险只影响首次访问，但在拿到移动端数据前，不能把「B 已验证」当成全平台结论。
...
| 距离代理 | 使用 DESIGN.md §9 的唯一 M3d manifest：固定三类语料、seed、完整 solver profile、solver/table fingerprint、最低 coverage 与 go/no-go 阈值；统一报告 Spearman ρ、MAE、逆序率、最短解方向一致率和 Lipschitz 违规率。随机游走只作对照；失败则整体切换指标版本，不逐样本混用 PDB。**第一轮已执行并判定 no-go**（逆序率 0.0591 > 0.05、Lipschitz 违规率 0.1583 > 0.10） |
| 进度指标 | 换版必须重新预注册：新 manifest 要写明替换了哪一版及原因（含前一版指纹）、语料、seed、门槛与门槛的类型，并在**同一批状态**上与被替换的指标同场比较。**第二轮 `structural-cubies-v1` 已执行并判定 go**，见「M3d 实测（第二轮）」 |
...
**尚未测量**：移动设备（§2.6 的另一半证据）；资产解码路径（方案 A 未采用，故无此路径）。M3c、M3d 见下两节。
...
最优解校准与距离代理验证都不是走过场。预注册门槛写的是「不过就整体换指标」，而 **M3d 第一轮跑完确实没过**，于是真的整体换了：`progress_score` 现在是 `structural-cubies-v1`（归位块数），第二轮预注册并判定 go。两轮实测都在下面，被否决的那一版连同它的脚本一起留在仓库里——没有它，「新的这版好在哪」就只是一句断言。
...
同一参考硬件（i7-9750H / WSL2 / Node 22.23.2，插电）。三类语料、seed、完整 solver profile、两个 fingerprint、最低 coverage 和全部 go/no-go 阈值在开跑之前写进 `src/metrics/manifest.ts`，并散列成 `M3D_FINGERPRINT = sha256:0781cb57…`。**阈值行本身也在被散列的文本里**，所以事后调一个门槛会连指纹一起改掉，这正是预注册要防的事。`npm run validate:proxy` 全程 37.8 s，两次运行除 `elapsedS` 外逐字节相同。

**结论：no-go。八条门槛过六条，逆序率与局部一致性两条不过。§6.5 的 `progress_score` 不能按现定义启用。**

| 门槛 | 实测 | 标准 | |
|---|---|---|---|
| coverage（A / B / C 三类各自） | 1.000 / 1.000 / 1.000 | ≥ 0.99 | ✅ |
| Spearman ρ | 0.9201 | ≥ 0.90 | ✅ |
| MAE | 1.227 步 | ≤ 1.5 | ✅ |
| 逆序率 | **0.0591** | ≤ 0.05 | ❌ |
| 最短解方向一致率 | 0.9611 | ≥ 0.90 | ✅ |
| Lipschitz 违规率 | **0.1583** | ≤ 0.10 | ❌ |

**A 类·已知真距离**（k=1..9 各 100 例共 900 个状态；参考值是 M3c 双向搜索给出的 `d*`，不是打乱步数）：

| d* | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|---|---|
| 样本 | 100 | 100 | 100 | 100 | 102 | 100 | 99 | 101 | 98 |
| 平均误差 | 0 | 0 | 0.26 | 1.02 | 1.29 | 0.84 | 1.70 | 2.29 | **3.68** |
| 最大误差 | 0 | 0 | 8 | 10 | 11 | 10 | 12 | 11 | 12 |

误差**全部是高估**——MAE 1.227 与有符号平均误差 1.227 完全相等，一例低估都没有。这是应该的：代理是解长度，天然是真距离的上界，低估会直接说明求解器错了。节点 P50 46 / P95 7,497 / max 847,541，远低于 64M 预算，所以没有一个样本是被预算截断的。

顺带补上 M3c 留的一个空：M3c 的语料里每一例都恰好 `d* = k`，本 seed 下不是——900 例里 `d*=5` 有 102 例、`d*=9` 只有 98 例，说明少数 k 步打乱的真距离小于 k。参考值取 `d*` 而不是 k，这个差别不影响结论。

**B 类·轨迹**（40 条，1,239 个轨迹点）：coverage 100%，2 条在 30 步内复原。**受指导的一步下降率 90.03%（893 步），滑步的下降率只有 8.50%（306 步）**——十倍差距，代理确实在跟踪某种真实的东西，不是噪声。`progress_score` 终值 P50 0.619，`best_progress` P50 0.700，**40 条里有 16 条 best 高于终值**，`best_progress` 这个指标本身是有必要的。轨迹尾部漂进最优求解器射程内的 234 个点有真距离：ρ = 0.9635、MAE = 0.363，比 A 类整体好得多，因为尾部点集中在近端。

**C 类·相邻状态对**（600 对；真距离在一步之内必然只差 1，这一类不需要 oracle，是 10–21 步区间唯一拿得到的证据）：

| 基态 | 代理 P50 | Lipschitz 违规率 | 最大 \|Δ\| |
|---|---|---|---|
| 均匀随机（≈18 步） | 21 | 0.047 | 3 |
| 6 步打乱 | 7 | 0.080 | 12 |
| 10 步打乱 | 16 | **0.293** | 12 |
| 14 步打乱 | 21 | **0.213** | 9 |

**对照·无方向随机游走**（20 条 × 20 步，从复原态出发，18 个走法均匀取样、不做任何过滤，所以允许自己撤销自己）：每步平均代理 0 → 1 → 1.8 → 3 → 3.65 → 4.9 → 6.2 → 6.7 → 9.7 → 10.75 → 10.8 → 12.4 → 13.55 → 14.35 → 14.55 → 16.55 → 18.65 → 18.45 → 18.35 → 18.7 → 19.35，与步数的 ρ = 0.8371（上升 249 / 下降 54 / 持平 97）。对照不要求单调，它在这里的作用是给 B 类的下降提供反面：无方向游走里代理不降反升，然后**在第 16 步左右压到 18.5 附近就不动了**。

**诊断**。两个机制，都能从上面的数字直接读出来：

1. **首解即返回。** 搜索在找到第一条 ≤ `targetLength`（21）的解时就停，它并不在总长度上做迭代加深。对一个已经很接近复原的魔方，第一条把它带进 G1 的 phase-1 路线常常配一条比整条最优解还长的 phase-2 尾巴。所以误差是单边的、d ≤ 2 时为零、并随 d 单调变大（0 → 3.68）。
2. **饱和。** 14 步打乱的代理 P50 是 21，均匀随机状态的代理 P50 也是 21——**代理分不出 14 步远和 18 步远的状态**。对照游走在第 16 步压到 18.5 不动是同一件事的另一面。

两个机制在 d ≈ 10–14 相遇，Lipschitz 违规率就堆在那里：10 步打乱 29.3%。均匀随机那一行的 4.7% 不是准确，是代理被顶在天花板上动不了——这一行低恰恰是坏消息，不是好消息。

**幸存的部分**，同样要如实记：`optimality_ratio` **完全不受影响**，它的分母是 M3c 的真最优，压根不经过这个代理。d ≤ 4 的代理是精确的。§6.5 那套 null / coverage 语义全部按设计工作：三类语料 coverage 都是 100%，没有一个样本被静默补零，`best_progress` 抓到了 16 条「接近过又走丢」的轨迹。方向一致率 96.11%，以及受指导步与滑步 90.0% : 8.5% 的对比，说明这个代理作为**方向**信号是好的，坏在当**距离**用。

**为什么文档自带的后备方案不解决问题。** DESIGN.md §9 预设的替代是「角块/棱块 PDB 下界」，但：

- M3a 的四张剪枝表直径是 9 / 9 / 14 / 12，而且每张只界定一个阶段。phase-1 的 PDB 下界在 9 就封顶，比现在这个代理饱和得**更早**。
- 完整角块 PDB（`8! × 3⁷` = 88,179,840 条）在 HTM 下直径是 11，同样远在 18 以下。
- 更根本的原因在球的层数里：1 / 18 / 243 / 3,240 / 43,239 / 574,908 每层约 ×13，而全群是 4.33 × 10¹⁹，所以绝大多数状态就挤在 17、18 两层。**哪怕有一个精确的距离 oracle，「距离比值」这个形状本身在 benchmark 真正会走到的状态上也几乎没有动态范围**——模型做完两层，离复原仍有十几步。

所以问题不在「哪个下界更紧」，在于 `progress_score` 不该是距离比值。下一版指标的候选（这是 §6.5 的定义变更，属于待决项，M3d 只负责给证据，不替它做主）：

- **(a) 结构进度**：已归位且定向正确的块数（0..20），或 §3.2 的七阶段前缀阶梯。动态范围满、便宜、不依赖求解器，代价是它不是距离。
- **(b) 保留距离比值，但只在被验证过的 d ≤ 9 上报告**，并如实给出 coverage——在真实任务上这个 coverage 会接近 0。
- **(c) 组合指标**：结构进度为主、近端换成真距离。

证据倾向 (a)，**最终也选了 (a)**：`structural-cubies-v1`，第二轮预注册并通过，见下一节。切换是整体的，全样本同一把尺子，没有逐样本混用。

**与手册的偏差，如实记录：**

- **B 类不是模型轨迹。** M4 还不存在，拿不到真实 trace，所以这一类是 manifest 里明确声明的替身 `solver-guided-walk-with-slips-v1`：走真实解的首步，以 `slipRate = 0.25` 的概率改走随机一步。它复现的是「大体知道在干什么、偶尔滑一下」这个形状，不是一个模型。**M4 落地后必须拿真实 trace 重跑这一类**，本节的 B 类结论在那之前不转移到模型身上。
- **阈值写定之前跑过一次约 60 样本的试点**，用途只有两个：确认三类语料跑得起、确认这几个统计量是有信息量的。阈值本身的推导写在 `manifest.ts` 的注释里，是从「`progress_score` 需要多准」倒推的；判定用的语料换了 seed。
- 统计量（Spearman ρ 的并列名次修正、逆序率、MAE/有符号误差、Lipschitz）都在 `src/metrics/statistics.ts` 里，有已知答案的单测——一个算错的相关系数和一个不合格的代理在输出里长得一模一样，而只有前者能被独立验证。

**代码覆盖率**：`src/metrics` 三个文件语句/分支/函数全部 100%。`cube-core` 整体 97.40% / 94.47%，达标。
...
同一参考硬件。第二份预注册 manifest 在 `src/metrics/manifest.ts` 后半段，`M3D_STRUCTURAL_FINGERPRINT = sha256:1f2639b4…`。被散列的文本里包含**第一轮的指纹**（`round1=sha256:0781cb57…`），所以「这一版替换了哪一版、因为什么」是散列的一部分而不是旁边的注释。`npm run validate:progress` 全程 31.2 s。

**结论：go。六条门槛全过。`progress_score` 换成 `structural-cubies-v1` 并启用。**

新定义：

```
placed(s)              = 二十个块里位置与朝向都正确的个数
progress_score(s0, s)  = clip( (placed(s) - placed(s0)) / (20 - placed(s0)), 0, 1 )
```

`placed(s0) = 20` 时为 null（任务一开始就是复原态，没有「剩余工作」可取分数）。「一个块算对」的判据与 §6.5 的 `cubie_accuracy` 完全同一条——位置和朝向都对，报告里只应该有一种「块对了」的意思。中心块不计分但**必须在原位**，否则拒绝：`isSolved` 是相对中心判定的，会把整体旋转过的复原态判为已复原，而按块计数会判为一个块都没归位；两个都不算错，正因为如此这个分歧必须断掉而不是挑一个。

**这一轮最重要的是下面这张表**——六个结构层级各 200 个状态，两套指标在**同一批状态**上算：

| 层级 | `placed` | 新 `progress_score` | 旧代理解长 |
|---|---|---|---|
| 打乱 | 0.80 | 0.040 | 20.68 |
| 底面十字 | 4.87 | 0.243 | 20.60 |
| 第一层完成 | 8.74 | 0.437 | 20.25 |
| 前两层完成 | 12.78 | 0.639 | 19.20 |
| 顶面定向完成 | 14.02 | 0.701 | 12.63 |
| 复原 | 20.00 | 1.000 | 0 |

**最右一列就是第一轮为什么被否决。** 整整一个第一层做完，旧代理只从 20.68 走到 20.25——四分之一步，折算成 `progress_score` 约 0.02。新指标在同一批状态上给到 0.437。层级 Spearman ρ：结构 **0.9765**，代理 0.8254。

层级语料不是解出来的，是在块层直接构造的：钉住一组块，其余在三条合法性约束（置换奇偶一致、`co` 和模 3、`eo` 和模 2）下随机，构造完再**独立**断言「被钉住的块确实到位」并跑一遍 `assertValidState`——不拿被测指标去检查自己的语料。层级名取自 §3.2 的阶段阶梯，和教程里是同一件事。

**六条门槛**：

| 门槛 | 类型 | 实测 | 标准 | |
|---|---|---|---|---|
| `score = 1` ⟺ 复原 | 回归 | 0 例外 / 25,640 个状态 | 0 例外 | ✅ |
| 层级单调 | 回归 | 0.80 < 4.87 < 8.74 < 12.78 < 14.02 < 20 | 严格递增 | ✅ |
| 层级 Spearman ρ | 回归 | 0.9765 | ≥ 0.90 | ✅ |
| 均匀随机均值 | **预测** | 0.8282（偏差 0.0051） | 20/24 ± 0.15 | ✅ |
| `\|Δplaced\| ≤ 8` | 回归 | 0 / 10,000 违规 | 0 违规 | ✅ |
| 强于被替换的指标 | **比较** | 0.9765 > 0.8254 | 严格大于 | ✅ |

**这一轮的门槛和第一轮不是同一种东西，manifest 里写明了这一点。** 代理那一轮问的是经验问题——解长度跟不跟得住真距离——答案可能两边倒。结构指标的多数行为由定义决定，所以六条里三条是回归守卫（只有实现坏了才会掉）。真正带风险的是两条：

- **闭式预测**：5,000 个均匀随机状态的 `placed` 均值实测 **0.8282**，预测 20/24 = 0.8333，差 0.0051。（一个角块同时到位且不扭是 `1/8 × 1/3`，一个棱块是 `1/12 × 1/2`，八角加十二棱正好 20/24。）这是唯一一条能被数据推翻的预测，样本也因此取全场最大。分布：2,169 个状态一个块都没归位，最多的一个有 6 个。
- **必须强于被替换的指标**：这是本轮真正的检验。第一轮死于饱和，一个同样饱和的替代品毫无意义，所以两套指标在同一批层级状态上算 ρ 并直接比。

**局部界**：10,000 对相邻状态，`|Δplaced| ≤ 8` **零违规**，实测最大 6。这条界是走法集的定理（四分之一转轮换四角四棱，半转各交换两对），不是对搜索的指望，容差因此是零而不是 10%。每步平均扰动：均匀随机基态 0.52 个块，6 步打乱基态 0.917 个。

**对照与已知短板，必须写清楚：**

- **无方向游走**（200 条 × 30 步，从复原态出发）：`placed` 走 20 → 12 → 7.41 → 5.15 → 3.65 → … → 第 14 步左右压到 0.85 附近不再动。**这个指标在离复原十几步以外同样是饱和的**，只不过饱和在 0 那一端。
- **轨迹类**（40 条替身轨迹，manifest 里预先声明「只报告、不设门槛」）：终值均值 0.115，`best_progress` 均值 0.194，**40 条里 27 条走到一半时仍然是 0**，24 条 best 高于终值，0 条复原。

两条是同一件事：替身轨迹走的是 Kociemba 解，那条路上直到最后几步没有任何块回家。这是指标按定义工作而不是失灵——一条还没让任何一个块归位的不透明路径，确实没产出裁判能验证的东西。但同样要如实说：**第二轮因此拿不到「真实解题轨迹」这一类的正面证据**，得等 M3.5 能生成分层解法之后才补得上，届时这一类要重跑。

两轮的压缩位置正好相反，这才是选它的理由：旧代理压缩在**进展真的发生的那一段**（第一层做完只动 0.02），新指标压缩在**没有可验证进展的那一段**（离复原十几步、一个块都没归位）。压缩在后面那一段是可以接受的。

**一处要主动交代的刻度问题**：前两层完成 0.639 → 顶面定向完成 0.701，只值 0.06，而人会觉得 OLL 是一大步。原因是这个刻度按「归位的块数」算，而顶面定向不让任何一个块归位。没有为此加权：任何权重都得另外找依据，而和 `cubie_accuracy` 保持同一条判据的价值更高。这一栏摆在表里，读报告的人自己能看见。

**成本**：每次打分 **0.0006 ms**，不需要表也不需要求解器。旧代理是 28.5 ms 加 3.62 MiB 常驻表。这不是附带好处：`progress_score` 要在每个轨迹点上读，赛道 B 一个任务最多 30 个点，一份几百任务的报告差着四个数量级。

**代码覆盖率**：`src/metrics` 五个文件语句/分支/函数全部 100%。
...
| **M3d** ✅ | **距离代理验证 profile**（DESIGN.md §9 待决 1） | 两轮都已执行：第一轮**否决**了 Kociemba 解长代理，第二轮**通过**了 `structural-cubies-v1`。`progress_score` 已启用 |
| **M3.5** | 模块三：策略表、七阶段 policy、公式变体、高亮/fallback、step coordinator、教程 UI | 可跟练、提示、演示的教学模式 |

依赖关系不是简单按小数顺序串行：

- `M3a → M3b → M3d` 是 benchmark 指标关键路径；M3d 两轮都已完成，`progress_score` 有了通过预注册验证的实现，**这条关键路径已经打通**，M4 可以带着正式指标开发
...
**当前 M3b/M3c/M3d 状态（2026-08-20）：** 两阶段搜索、朝向入口（§2.0）、可暂停内核、四种返回分支与确定性均已完成并实测（见 §2.9）；Worker、client、IndexedDB 表缓存与游戏内「Solve」按钮已接通。**M3c 双向最优解已落地并实测**（见「M3c 实测」），`optimality_ratio` 的分母现在算得出来。**M3d 两轮都已执行完毕**：第一轮否决了 Kociemba 解长代理，第二轮的 `structural-cubies-v1`（归位块数）通过全部六条门槛，`progress_score` 已启用。`/metrics` 子路径同时提供两版实现——被否决的那一版留着，是为了让「新的这版强在哪」可以在同一批状态上量出来。
