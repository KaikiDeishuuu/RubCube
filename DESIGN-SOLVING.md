...
>
> 本文中的状态空间与表大小来自明确的坐标基数推导；球层数量、表直径和公式保持性是实现必须复现的验收常数。M3a 的六张移动表和四张剪枝表已由穷举 oracle 固化，9/9/14/12 是当前实现复现的直径；M3c 的球层数量 1/18/243/3,240/43,239/574,908 也已由实现复现。**M3d 的距离代理验证已经跑完，结论是否决**（见「M3d 实测」），所以 §6.5 的 `progress_score` 在本文里一律按「候选、未启用」读。M3.5 教程仍未实现，不能把对应设计值提前写成实测结果。
...
cube-core (state / moves，默认入口只导出规则 API)
  ...
  └── metrics/       ← §6.5 的 progress_score 与 M3d 预注册 manifest，依赖 solver，只给裁判使用
...
| 距离代理 | 使用 DESIGN.md §9 的唯一 M3d manifest：固定三类语料、seed、完整 solver profile、solver/table fingerprint、最低 coverage 与 go/no-go 阈值；统一报告 Spearman ρ、MAE、逆序率、最短解方向一致率和 Lipschitz 违规率。随机游走只作对照；失败则整体切换指标版本，不逐样本混用 PDB。**已执行，判定 no-go：逆序率 0.0591 > 0.05、Lipschitz 违规率 0.1583 > 0.10，见「M3d 实测」** |
...
最优解校准与距离代理验证都不是走过场。预注册门槛写的是「不过就整体换指标」，而 **M3d 跑完确实没过**（见「M3d 实测」），所以 §6.5 的 `progress_score` 现在处于「已否决、待换实现」的状态，不是「已启用」。同一份数据还表明预设的替代方案（PDB 下界）比被否决的代理饱和得更早，替换方案因此仍是待决项。
...
证据倾向 (a)。无论选哪个，都必须整体切换、全样本同一把尺子，不能逐样本混用。
...
| **M3d** ✅ | **距离代理验证 profile**（DESIGN.md §9 待决 1） | 已执行并**判定 no-go**：`progress_score` 的 Kociemba 实现被否决，正式报告不得使用；替换实现待定 |
| **M3.5** | 模块三：策略表、七阶段 policy、公式变体、高亮/fallback、step coordinator、教程 UI | 可跟练、提示、演示的教学模式 |

依赖关系不是简单按小数顺序串行：

- `M3a → M3b → M3d` 是 benchmark 指标关键路径；M3d 已给出结论且是否决，所以 M4 runner 可以继续开发，但**正式报告在换掉 `progress_score` 的实现之前不能发布**
...
**当前 M3b/M3c/M3d 状态（2026-08-20）：** 两阶段搜索、朝向入口（§2.0）、可暂停内核、四种返回分支与确定性均已完成并实测（见 §2.9）；Worker、client、IndexedDB 表缓存与游戏内「Solve」按钮已接通。**M3c 双向最优解已落地并实测**（见「M3c 实测」），`optimality_ratio` 的分母现在算得出来。**M3d 的验证 profile 也已执行完毕，结论是 no-go**（见「M3d 实测」）：`/metrics` 子路径与预注册 manifest 已落地，但它验证的那个代理没过门槛，`progress_score` 因此仍未启用。
